...
main = do
  ...
  case mwin of
    ...
    Just win -> do
      ...
      (GLFW.setKeyCallback win . Just) (\_ _ _ _ _ -> atomically (modifyTVar shouldSwap not))
      (vao, prog) <- setup
...
setup = do
  vao <- newVAO
  bindVAO vao
  vsource <- readFile =<< getDataFileName "blending.vert"
  fsource <- readFile =<< getDataFileName "blending.frag"
